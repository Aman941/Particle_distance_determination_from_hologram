    ...
    for col = 1:cols
        for row = 1:rows
            if(intensity_map(row,col) > intensity_img(row,col))
                ...
                intensity_dist(row,col) = (0.9*z+(0.2*z/frame_no)*n);
            end
            if(tenen_map(row,col) < tenen_img(row,col))
                tenen_map(row,col) = tenen_img(row,col);
                tenen_dist(row,col) = (0.9*z+(0.2*z/frame_no)*n);
            end
        end
    end
end
...
for col = 1:cols
    for row = 1:rows
        if(intensity_map(row,col) < intensity_thresh)
            counter = counter +1;
            dist_sum = dist_sum + intensity_dist(row,col);
        end
    end
end
dist = dist_sum/counter;
...
for col = 1:cols
    for row = 1:rows
        if(tenen_map(row,col) > tenen_thresh)
            ...
            tenen_dist_sum = tenen_dist_sum + tenen_dist(row,col);
        end
    end
end
